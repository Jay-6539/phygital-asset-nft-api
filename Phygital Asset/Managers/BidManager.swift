...
class BidManager {
    ...
    func createBid(request: CreateBidRequest, bidderUsername: String) async throws -> Bid {
        Logger.debug("💰 Creating bid: \(request.bidAmount) echo for record \(request.recordId)")
        ...
        // 冻结出价金额
        EchoManager.shared.freezeEcho(bid.bidAmount, for: bidderUsername)
    ...
    func counterOffer(bidId: UUID, counterAmount: Int, message: String?) async throws {
        Logger.debug("🔄 Counter offer: \(counterAmount) echo for bid \(bidId)")
        ...
        Logger.success("✅ Counter offer sent: \(counterAmount) echo")
    }
    ...
    func acceptBid(bidId: UUID, contactInfo: String, isBidder: Bool) async throws {
        ...
        if shouldComplete {
            ...
            Logger.debug("💸 Transferring \(finalAmount) echo from @\(bidData.bidderUsername) to @\(bidData.ownerUsername)")
            
            // 重要：先执行Echo转账（deductEcho会检查总余额，包括frozen部分）
            // 然后再解冻，避免余额不足错误
            try EchoManager.shared.transferEcho(
                amount: finalAmount,
            ...
            EchoManager.shared.unfreezeEcho(bidData.bidAmount, for: bidData.bidderUsername)
            
            Logger.success("💰 Echo transfer completed!")
    ...
    private func rejectOtherBidsForAsset(
        ...
        for bid in otherBids {
            EchoManager.shared.unfreezeEcho(bid.bidAmount, for: bid.bidderUsername)
            Logger.debug("   ❌ Rejected bid from @\(bid.bidderUsername) (\(bid.bidAmount) echo)")
        }
    }
