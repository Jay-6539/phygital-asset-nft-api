...
struct BidInputView: View {
    ...
    @State private var availableEcho = 0
    
    var canSubmit: Bool {
        guard let amount = Int(bidAmount), amount > 0 else { return false }
        return amount <= availableEcho
    }
    ...
    var body: some View {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            // 余额不足提示
                            if let amount = Int(bidAmount), amount > availableEcho {
                                HStack(spacing: 6) {
                                    Image(systemName: "exclamationmark.circle.fill")
                                        .font(.caption)
                                    Text("Insufficient credits. You need \(amount - availableEcho) more credits.")
                                        .font(.caption)
                                }
                                .foregroundColor(.red)
        ...
        .onAppear {
            // 加载用户可用Echo余额（总额 - 冻结）
            availableEcho = EchoManager.shared.getAvailableEcho(for: currentUsername)
            Logger.debug("💰 Available credits for bid: \(availableEcho)")
        }
    }
