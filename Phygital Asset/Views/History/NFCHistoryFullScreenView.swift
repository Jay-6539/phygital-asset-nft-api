...
struct NFCHistoryFullScreenView: View {
    ...
    private func loadHistory() {
        ...
        Task {
            ...
            // 从 threads 表获取
            do {
                Logger.debug("📋 查询 threads 表...")
                fetchedCheckIns = try await BuildingCheckInManager.shared.getCheckInsByNFC(nfcUuid: nfcUuid)
                Logger.success("📋 从 threads 获取到 \(fetchedCheckIns.count) 条记录")
            } catch {
                Logger.error("📋 从 threads 获取失败: \(error.localizedDescription)")
            }
            
            // 从 oval_office_threads 表获取
            do {
                Logger.debug("📋 查询 oval_office_threads 表...")
                fetchedOvalCheckIns = try await OvalOfficeCheckInManager.shared.getCheckInsByNFC(nfcUuid: nfcUuid)
                Logger.success("📋 从 oval_office_threads 获取到 \(fetchedOvalCheckIns.count) 条记录")
            } catch {
                Logger.error("📋 从 oval_office_threads 获取失败: \(error.localizedDescription)")
            }
